 ...
 * İş deneyimi yoksa veya eşiği geçmiyorsa G1 veya H önerilir.
 */
export function grupHesapla(isDeneyimTL: number): {
  ...
  // Grupları iş deneyimine göre büyükten küçüğe sırala
  // Kaynak: "Grup için gerekli asgari iş deneyim tutarı" tablosu (2026)
  ...
  // Eşiği geçen en yüksek grubu bul
  ...
  // F1 eşiğini geçemedi — G1 öner, H minimum
...
// Kaynak: PDF EK tablosu
// ─────────────────────────────────────────────

export interface YapiSinifi {
  sinif: string;   // örn: "III.B", "IV.A"
...
/**
 * Yapı sınıfı ve m2'ye göre iş deneyim tutarını hesaplar.
 * İskanlı yapı m2'si x birim fiyat formülü kullanılır.
 */
export function isDeneyimHesapla(
  yapiSinifi: string,
  m2: number,
): number {
...
/**
 * Birden fazla yapının toplam iş deneyimini hesaplar.
 */
export function toplamIsDeneyimHesapla(
  yapilar: Array<{ yapiSinifi: string; m2: number }>
): number {
  return yapilar.reduce((toplam, yapi) => {
    return toplam + isDeneyimHesapla(yapi.yapiSinifi, yapi.m2);
  }, 0);
}
...
/** Hizmet paketini konum ve senaryoya göre filtreler */
export function uygunPaketleriGetir(
  istanbul: boolean,
  isDeneyimiVar: boolean,
): HizmetPaketi[] {
  return Object.values(HIZMET_PAKETLERI).filter(paket => {
    ...
    // İstanbul dışı + iş deneyimi varsa uzaktan destek sunulmaz (sadece hesaplama)
...
export type BasvuruDurumu =
  | "analiz_bekleniyor"      // Wizard tamamlandı, analiz yapılmadı
  | "analiz_tamamlandi"      // Grup belirlendi
  | "hizmet_secildi"         // Müşteri paketi seçti, ödeme bekleniyor
  | "odeme_alindi"           // Ödeme onaylandı
  | "evrak_toplaniyor"       // Müşteriden evrak isteniyor
  | "evrak_tamamlandi"       // Tüm evraklar yüklendi
  | "basvuru_yapildi"        // Bakanlığa başvuru iletildi
  | "basvuru_onaylandi"      // Belge alındı
  | "basvuru_reddedildi"     // Eksik/ret
  | "yukseltme_bekliyor";    // Mevcut belgeli, yükseltme süreci başlatıldı
...
// Kaynak: Görsel 3 — Adi veya İş Ortaklığı Şartları tablosu
// ─────────────────────────────────────────────

/**
 * Talep edilen gruba göre adi ortaklık şartlarını döndürür.
 *
 * Çıkarım örnekleri:
 *  - F grubu almak için: pilot G grubu + özel ortak H (veya daha üstü)
 *  - E1 grubu almak için: pilot E1 + özel H
 *  - H grubunun dahil olduğu ortaklık en yüksek E1 alabilir (H tüm gruplarda özel ortak olabilir)
 *  - H+H ortaklığı sadece H grubu alabilir
 */
...
/**
 * İki firmanın mevcut gruplarıyla adi ortaklıkta hangi grupları alabileceğini hesaplar.
 *
 * Kural: Talep edilen grubun pilot ve özel ortak şartlarını her ikisi karşılıyorsa
 *        o grup alınabilir. Birden fazla grup karşılanabilir — hepsi döndürülür.
 *
 * Önemli: Adi ortaklıklarda banka referans mektubu ve EK-2 ciro/bilanço şartları ARANMAZ.
 *
 * Örnek: G1 + H → pilot G1, özel H → hem G1 hem G alınabilir (ikisi de şartı karşılar).
 *        E1 + H → pilot E1, özel H → E1 alınabilir.
 *        H  + H → pilot H,  özel H → sadece H alınabilir.
 */
export function ortaklikGrubuHesapla(
  ...
  maksimumGrup: GrupKodu;
  uygunGruplar: GrupKodu[];   // alınabilecek TÜM gruplar (tercih seçeneği)
  pilotFirma: GrupKodu;
  ozelFirma: GrupKodu;
  bankaReferansiGerekli: false;       // adi ortaklıkta hiçbir zaman aranmaz
  bilancoCiroSartiGerekli: false;     // EK-2 ciro/bilanço şartları da aranmaz
  aciklama: string;
} {
  ...
  // Her iki firmanın karşıladığı TÜM grupları bul
  ...
  const maksimumGrup = uygunGruplar[0] ?? "H"; // en iyisi ilk sırada
...
// Kaynak: Görsel 4 — Grubun üstlenebileceği iş tutarı / yapı sınıfı birim fiyatı
// ─────────────────────────────────────────────
...
/**
 * Bir grubun belirli yapı sınıfında kaç m2 iş yapabileceğini hesaplar.
 * Formül: maxIsTutariTL / birimFiyat
 *
 * Örnek: grupM2Hakki("G1", "III.B") → 4.412 m2
 */
...
/**
 * Bir grubun tüm yapı sınıfları için m2 haklarını döndürür.
 */
